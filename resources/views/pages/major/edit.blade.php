        ...
        <div class="flex">
            <div class="w-1/2">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    <a href="{{ route('major.list') }}" class="text-red-500">{{ __('Branş') }}</a> > {{ __('Düzenle') }}
                </h2>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('major.update', $major->id) }}" method="POST" autocomplete="off" class="mt-4" id="majorForm">
                        @csrf
                        @method('PUT')
                        <div class="flex">
                            <div class="w-1/2 p-3">
                                <x-input-label  for="name" class="block text-gray-700">{{ __('Branş Adı') }}</x-input-label>
                                <x-text-input name="name" type="text" id="name" class="form-input w-full" value="{{ $major->name }}"  />
                            ...
                            <div class="w-1/2 p-3">
                                ...
                                <select name="status" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="active" @if($major->status == 'active') selected @endif>
                                        {{ __('active') }}</option>
                                    <option value="inactive" @if($major->status == 'inactive') selected @endif>
                                        {{ __('inactive') }}</option>
                                </select>
