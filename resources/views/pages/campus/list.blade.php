    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('campus.filter') }}" method="POST" autocomplete="off" class="mt-4">
                    <div class="border-b-2 border-gray-400 pb-3 mb-15 text-gray-700">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="text-right">
                                    ...
                                    <a href="{{ route('campus.list') }}" class="mr-3 text-red-600"><span class="bi bi-x-circle"> </span>{{__('Filtreleri temizle')}}</a>
                                    @endif
                        ...
                        <table class="table-auto w-full">
                            ...
                            @foreach ($campuses as $campus)
                                <tr>
                                    ...
                                    <td class="text-center py-2">{{ $campus->name }}</td>
                                    <td class="text-center"><span class="bg-{{ $campus->status == 'active' ? 'green' : 'red' }}-500 text-white py-1 px-2 rounded-full">{{ __($campus->status) }}</span></td>
                                    <td class="text-center">
                                        <a href="{{ route('campus.edit', $campus->id) }}"><i class="bi bi-pencil text-red-500"></i></a>
                                        <a href="{{ route('campus.delete', $campus->id) }}" onclick="event.preventDefault(); document.getElementById('inactive-form-{{ $campus->id }}').submit();">
                                            <i class="bi bi-trash text-red-500"></i>
                                        </a>
                                        <form id="inactive-form-{{ $campus->id }}" action="{{ route('campus.delete', $campus->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </td>
