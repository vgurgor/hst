        ...
        <div class="flex">
            <div class="w-1/2">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    <a href="{{ route('campus.list') }}" class="text-red-500">{{ __('Kampüs') }}</a> > {{ __('Düzenle') }}
                </h2>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('campus.update', $campus->id) }}" method="POST" autocomplete="off" class="mt-4" id="campusForm">
                        @csrf
                        @method('PUT')
                        <div class="flex">
                            <div class="w-1/2 p-3">
                            ...
                            <div class="w-1/2 p-3">
                                ...
                                <select name="status" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="active" @if($campus->status == 'active') selected @endif>
                                        {{ __('active') }}</option>
                                    <option value="inactive" @if($campus->status == 'inactive') selected @endif>
                                        {{ __('inactive') }}</option>
                                </select>
