    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('branch.filter') }}" method="POST" autocomplete="off" class="mt-4">
                    <div class="border-b-2 border-gray-400 pb-3 mb-15 text-gray-700">
                            <div class="flex justify-between items-center">
                                ...
                                <div class="text-right">
                                    @if(!empty($filter) && $filter)
                                        <a href="{{ route('branch.list') }}" class="mr-3 text-red-600"><span class="bi bi-x-circle"> </span>{{__('Filtreleri temizle')}}</a>
                                    @endif
                        ...
                        <div class="flex">
                            <div class="w-1/2 p-3">
                                <x-input-label  for="campus_id" class="block text-gray-700">{{ __('Kampüs Adı') }}</x-input-label>
                                <select name="campus_id[]" multiple="multiple" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                        ...
                        <table class="table-auto w-full">
                            ...
                            @foreach ($branches as $branch)
                                <tr>
                                    ...
                                    <td class="text-center py-2">{{ $branch->campus->name  }}</td>
                                    <td class="text-center py-2">{{ $branch->name }}</td>
                                    <td class="text-center py-2">{{ __($branchTypes[$branch->type]) }}</td>
                                    <td class="text-center"><span class="bg-{{ $branch->status == 'active' ? 'green' : 'red' }}-500 text-white py-1 px-2 rounded-full">{{ __($branch->status) }}</span></td>
                                    <td class="text-center">
                                        <a href="{{ route('branch.edit', $branch->id) }}"><i class="bi bi-pencil text-red-500"></i></a>
                                        <a href="{{ route('branch.delete', $branch->id) }}" onclick="event.preventDefault(); document.getElementById('inactive-form-{{ $branch->id }}').submit();">
                                            <i class="bi bi-trash text-red-500"></i>
                                        </a>
                                        <form id="inactive-form-{{ $branch->id }}" action="{{ route('branch.delete', $branch->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('PUT')
                                        </form>
                                    </td>
