        ...
        <div class="flex">
            <div class="w-1/2">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    <a href="{{ route('teacher.list') }}" class="text-red-500">{{ __('Öğretmen') }}</a> > {{ __('Düzenle') }}
                </h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100" style="min-height: 60vh;">
                    <form action="{{ route('teacher.update', $teacher->id) }}" method="POST" autocomplete="off" class="mt-4" id="teacherForm">
                        @csrf
                        @method('PUT')
                        <div class="flex">
                            <div class="w-1/2 p-3">
                                <x-input-label  for="first_name" class="block text-gray-700">{{ __('Adı') }}</x-input-label>
                                <x-text-input name="first_name" type="text" id="first_name" class="form-input w-full" value="{{ $teacher->first_name }}"  />
                                <x-input-error :messages="$errors->get('first_name')" class="mt-2" />
                            </div>
                            <div class="w-1/2 p-3">
                                <x-input-label  for="last_name" class="block text-gray-700">{{ __('Soyadı') }}</x-input-label>
                                <x-text-input name="last_name" type="text" id="last_name" class="form-input w-full" value="{{ $teacher->last_name }}"  />
                            ...
                            <div class="w-1/2 p-3">
                                ...
                                <select name="status" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                    <option value="active" @if($teacher->status == 'active') selected @endif>
                                        {{ __('active') }}</option>
                                    <option value="inactive" @if($teacher->status == 'inactive') selected @endif>
                                        {{ __('inactive') }}</option>
                                </select>
                                    ...
                                    @foreach ($branches as $key => $value)
                                        <optgroup label="{{$key}}">
                                            @foreach ($value as $id => $branch)
                                                <option value="{{ $id }}" @if(!empty($teacher->branches->pluck('id')->toArray()) && in_array($id, $teacher->branches->pluck('id')->toArray())) selected @endif>{{ $branch }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
